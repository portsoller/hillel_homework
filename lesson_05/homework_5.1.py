...
Маємо критерії пошуку у вигляді кортежу (рік ≥, об'єм двигуна ≥, ціна ≤). Напишіть код, який допоможе нам
отримати автомобілі, які відповідають критеріям пошуку. Автомобілі повинні бути відсортовані за зростанням ціни.
...
good_cars = []
for brand, characteristics in car_data.items():
    # good_cars = []
    year = characteristics[1]
    ...
    if year >= search_criteria[0] and engine >= search_criteria[1] and price <= search_criteria[2]:
        #good_cars.append(year)
        #good_cars.append(engine)
        #good_cars.append(price)
        good_cars.append((price, brand))
        print(year, engine, price)
# print(good_cars)

sorted_cars = sorted(good_cars)
print(sorted_cars[:5])
