...
@allure.feature("Перевірка підключення")
@allure.story("Назва сценарію")
@allure.title("Назва тесту")
@allure.description("Опис тесту")
def test_connection(db_connection):
    with allure.step("Перевірка статусу підключення до БД"):
        assert db_connection is not None
        assert db_connection.status == psycopg2.extensions.STATUS_READY

@allure.feature("Перевірка бази даних")
def test_crud_operations(db_connection):
    ...
    with allure.step("SELECT FROM products"):
    ...
    with allure.step("UPDATE products"):
    ...
    with allure.step("DELETE FROM products"):
